...
@login_required
def programa_modalidades(request):
    """Vista para gestionar modalidades por programa"""
    # Obtener todos los programas activos con su municipio
    programas_activos = Programa.objects.filter(
        estado='activo'
    ).select_related('municipio').order_by('municipio__nombre_municipio', 'programa')

    # Obtener todas las modalidades del sistema
    modalidades = ModalidadesDeConsumo.objects.all().order_by('modalidad')

    # Para cada programa, obtener sus modalidades activas
    programas_data = []
    for programa in programas_activos:
        modalidades_activas = ProgramaModalidades.objects.filter(
            programa=programa
        ...
        programas_data.append({
            ...
            'municipio_nombre': programa.municipio.nombre_municipio,
    ...
    return render(request, 'principal/programa_modalidades.html', context)


@login_required
@csrf_exempt
def api_guardar_programa_modalidades(request):
    """API para guardar cambios de modalidades por programa"""
    ...
    try:
        ...
        for programa_id, acciones in cambios.items():
            ...
            for modalidad_id in acciones.get('eliminar', []):
                ProgramaModalidades.objects.filter(
                    programa=programa,
